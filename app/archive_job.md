...
2. **Assemble `run_data.json`** (schema: see `archive_generate.py` docstring + git history example): `as_of` (ISO now), `snapshot_date` (YYYY-MM-DD), `trades[]`, `positions[]` (option legs with strikes + stocks with last price, accounts as last-4), `accounts[]`.
