...
`structure_key · created_at · updated_at · thesis · tags · rating(1-5) · mindset · notes · risk_plan · close_plan`
...
**Known gap as of 2026-07-23:** file is header-only. **M15's original plan (manual "Queue sync" button, a client-managed `radar_sync_requests.csv` Pop would download/replace/re-ask-Claude-about) was scrapped the same day** — walking through it end-to-end, it required Pop to download a file, replace it in Drive, come back in a later session, ask Claude in words, wait, download a second file, and replace it again just to see one ticker's data. That's not a sync mechanism, it's a manual relay race. **M16 replaces it:** a scheduled Claude task (same mechanism as the Friday archive courier, just a shorter interval — proposed every 30–60 min during market hours) pulls Pop's chosen watchlist automatically and commits `radar_snapshot.csv` — Pop just opens Radar and the data is there, no clicking or file-shuffling required. Adding a ticker outside the watchlist: tell Claude directly in a session, pulled and committed the same turn, no intermediate request file. Not yet created — needs the Robinhood MCP connector reconnected first (disconnected as of 2026-07-23) and Pop's watchlist/interval confirmed. First real step once the connector is back: inspect `get_scanner_filter_specs`/`run_scan`'s actual output shape (Robinhood's connector may already compute RSI/IV-rank — reuse rather than hand-derive), run one manual pull, cross-check against a known ticker, then turn the scheduled task on.
