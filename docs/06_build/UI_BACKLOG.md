...
- [x] Sync mechanism — "Queue sync" button on the Radar page (`radar_sync_requests.csv`, upsert by symbol, same download-and-replace pattern as every other write in the app). Deliberately Pop-initiated, not automatic: the browser can't call Robinhood directly (no client-side credentials, hard rule), so queuing a ticker is a request Claude picks up in a future session with the connector active. Replaces the earlier "recurring scheduled task" plan — Pop said explicitly this should be an option, not a mandate.
- [ ] **Still not shipped — the actual live pull.** Whenever a future session has the Robinhood connector reconnected: read `radar_sync_requests.csv` for pending tickers, inspect `get_scanner_filter_specs`/`run_scan`'s actual output shape (Robinhood's connector may already compute RSI/IV-rank — reuse rather than hand-derive), pull each pending ticker, write results into `radar_snapshot.csv`, mark requests `done`.
